...
public T[] toArray() throws ListException {
    if (count == 0) {
        throw new ListException("Cannot convert to array. List is empty.");
    }

    // Create a new array of the same type and size as L
    T[] arr = (T[]) new Object[L.length];

    // Copy all elements in L to the new array
    System.arraycopy(L, 0, arr, 0, count);

    return arr;
}
// ifEmpty method to check count of the list
